...
def animate(shot_id: str, keyframe: Path, prompt: str, cfg: I2VConfig, out_path: Path, *, progress_cb=None) -> I2VResult:
    ...
    docs/live-benchmark-plan.md). The offload mode from `cfg` is applied to the pipeline, plus VAE
    ...
    pipe = CogVideoXImageToVideoPipeline.from_pretrained(cfg.model, torch_dtype=dtype)
